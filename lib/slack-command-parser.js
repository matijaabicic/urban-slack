...
module.exports.parse = function (commandText){
  ...
  result.Command = commandText.replace(/--public/g, '').replace(/--private/g, '').replace(/—private/g, '').replace(/—public/g, '').replace(/--sfw/g, '').replace(/—sfw/g, '').replace(/—nsfw/g, '').replace(/--nsfw/g,'');
